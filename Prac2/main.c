...
int main(void){
  ...
  while (1)
  {
	...
	sprintf(buffer, "%02d-%02d-%d %02d:%02d:%02d %d \r\n", time1.dayofmonth, time1.month, time1.year, time1.hour, time1.minutes, time1.seconds, epochFromTime(time1));
	//sprintf(buffer, "%d \r\n", decToBcd(23));
...
void setTime (uint8_t sec, uint8_t min, uint8_t hour, uint8_t dow, uint8_t dom, uint8_t month, uint8_t year)
{
	...
	HAL_I2C_Mem_Write(&hi2c1, DS3231_ADDRESS<<1, FIRST_REG, REG_SIZE, set_time, 7, 1000);
...
void getTime (void)
{
	...
	HAL_I2C_Mem_Read(&hi2c1, (DS3231_ADDRESS << 1), FIRST_REG, REG_SIZE, get_time, 7, 1000);

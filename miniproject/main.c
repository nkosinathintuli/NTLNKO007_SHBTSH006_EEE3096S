...
uint32_t adc_val = 0;
uint32_t ldr_threshold = 4000;
//TO DO:
//TASK 1
//Create global variables for debouncing and delay interval

static uint32_t delay = 1000; // 1 seconds
...
uint32_t pollADC(uint32_t delay);
uint32_t ADCtoCRR(uint32_t adc_val);
void ADCtoBinary(uint8_t adc_val, uint8_t *binary);
void sendMessage(uint8_t *binary, uint8_t msg_type);
void receiveMessage(uint32_t delay);
...
int main(void)
{
  ...
  while (1)
  {
    ...
    // if(FLAG){
		...
		// ccr_val = ADCtoCRR(poll);
		// dc = 100 * ((float) ccr_val/(float) 47999);

		// //ADCtoBinary(dc, &binary);
		// //sendMessage(&binary, 1);

		//  sprintf(buffer, "%d \r\n", adc_val);
		//  HAL_UART_Transmit(&huart2, (uint8_t *)buffer, sizeof(buffer), 1000);
		// //FLAG = 0x00; // comment out on the receive side
    // }
...
/**
  * @brief  Poll the ADC.
  * @param  delay Delay between each ADC conversion
...
uint32_t pollADC(uint32_t delay){
	...
	HAL_ADC_Start(&hadc);
	uint32_t ADCValue = 0x00;
	if (HAL_ADC_PollForConversion(&hadc, delay) == HAL_OK){
		 ADCValue = HAL_ADC_GetValue(&hadc); /* Gets CH7 */
		 //HAL_ADC_PollForConversion(&hadc, delay);
	}
	HAL_ADC_Stop(&hadc);
...
/**
  * @brief  Convert ADC to CRR.
  * @param  adc_val ADC value
  * @retval the CRR value
  */ 
uint32_t ADCtoCRR(uint32_t adc_val){
	//TO DO:
	//TASK 2
	...
	float scale = 47999/(float) 4095;

	uint32_t crr = scale * adc_val;

	return crr;
}

/**
  * @brief  Convert ADC to 7 bit binary.
  * @param  adc_val ADC value
  * @param  *bin_array pointer to the array of 7 bits
  * @retval None
  */ 
void ADCtoBinary(uint8_t adc_val, uint8_t *binary){
  //TO DO:
  //TASK 3
  // Complete the function body
  //HINT: Use the modulo operator to get the binary value of each bit
  //HINT: Use the division operator to get the next bit
  //HINT: The binary array should be in the order: MSB, LSB
  //HINT: The binary array should be in the order: MSB, LSB
  uint8_t temp = adc_val;
  for (int i = 6; i >= 0; i--){
    binary[i] = temp % 2;
    temp = temp / 2;
  }
...
// 
/**
  * @brief  for each bit in the binary array, turn on the led if the bit is 1 
            and off if the bit is 0 delay for 500ms between each led turn 
            on/off 
  * @param  *binary pointer to the array of 7 bits
...
void sendMessage(uint8_t *binary, uint8_t msg_type) {
  ...
  // set LED off when done
  HAL_GPIO_WritePin(LD4_GPIO_Port, LD4_Pin, GPIO_PIN_RESET);
  //track samples sent
  if(msg_type == 1) noOfSamples++;
}

/**
  * @brief  poll the ADC at 250ms intervals and if greater than a certain 
            threshold, turn the LED on else turn the LED off
  * @param  *binary pointer to the array of 7 bits
  * @param  msg_type message type
  * @retval None
  */ 
void receiveMessage(uint32_t delay){
  //uint8_t binary[7];
  //ADCtoBinary(adc_val, binary);
  //sendMessage(binary, 1);
	int recieved[8] = {0,0,0,0,0,0,0,0};
	uint8_t start_count=0, end_count=0, record=0;
  int i = 0;
	HAL_GPIO_WritePin(LD4_GPIO_Port, LD4_Pin, GPIO_PIN_RESET);
  while(FLAG2){
    adc_val = pollADC(0);
    ...
    if (adc_val > ldr_threshold){
      HAL_GPIO_WritePin(LD4_GPIO_Port, LD4_Pin, GPIO_PIN_SET);
     if (record==0) {start_count++;}
     else if(record==1){recieved[i++]=1;}
    }
    else{
      HAL_GPIO_WritePin(LD4_GPIO_Port, LD4_Pin, GPIO_PIN_RESET);
     if(record==1){recieved[i++]=0;}
     else if(record ==0){start_count=0;}
    }
   if (start_count == 2){
     record = record==0?1:0;
     start_count = -2;
   ...
   if(i>7) {
     i = 0;
     record = 0;
     sprintf(buffer, "%d%d%d%d%d%d%d%d \r\n", recieved[0], recieved[1], recieved[2], recieved[3], recieved[4], recieved[5], recieved[6], recieved[7]);
      HAL_UART_Transmit(&huart2, (uint8_t*)buffer, sizeof(buffer), 1000);
   }
    HAL_Delay(delay);
  }
